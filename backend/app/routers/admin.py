...
async def update_batch(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update batch core data: {str(e)}")

    # 2. Then attempt automation (Autopilot) in a separate try block
    try:
        ...
        # Since core was flushed, we try to commit JUST the batch updates if automation failed
        try:
            await db.commit() 
            return {"status": "updated", "warning": f"Batch metadata saved, but session automation failed: {str(e)}"}
        except:
            await db.rollback()
